...
5. **Do not sign out or close this tab.**
...
![Get Started](../dashboard/assets/devleads_setup_images/firebase/firebase_get_started_auth.png)
...
4. **Users** tab → Add your first user
...
npm run dev
```
![Terminal](../dashboard/assets/terminal_port_5000.png)
...
1. **Schedule Consultation Link** ([`emailNotification.js:446`](../server/utils/emailNotification.js#L446))

   - Replace `href="#"` with your actual scheduling URL
   - Example: `href="https://cal.com/your-business/consultation"`

2. **Support Email Address** ([`emailNotification.js:453`](../server/utils/emailNotification.js#L453))
...
3. **Business Name** ([`emailNotification.js:458`](../server/utils/emailNotification.js#L458))
...
3. **CRITICAL: Choose Docker Environment**
...
7. **⚠️ REQUIRED**: Click `Edit` then add `DOMAIN` environment variable on RENDER with your deployment domain name __AFTER__ your first deployment and select **"Save, Rebuild, and Deploy"**.  It will be automatically generated by Render (or other host).
...
1. **Update CORS settings in `server/server.js`** for production security:
   ...
   **IMPORTANT:** You need to modify two CORS configuration blocks:
   ...
   - **UNCOMMENT Production CORS** ([line 33](../server/server.js#L33) through [line 62](../server/server.js#L62)):
     - Start at [line 33](../server/server.js#L33) with the comment `// PRODUCTION CORS configuration - uncomment for production use`
   ...
   - **COMMENT OUT Development CORS** ([line 66](../server/server.js#L66) through [line 71](../server/server.js#L71)):
     - Start at [line 66](../server/server.js#L66) with the comment `// Development CORS - comment out in production`
   ...
   - **DO NOT MODIFY** [line 74](../server/server.js#L74) through [line 76](../server/server.js#L76):
